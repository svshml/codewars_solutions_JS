...
the next coding meetup that you are organising. The list is ordered according to who signed up first.
...
function getFirstPython(list) {
  let res = 'There will be no Python developers';
  for(let i=0; i< list.length;i++){
    if (list[i].language == 'Python'){
      res = `${list[i].firstName}, ${list[i].country}`;
      break;
      }
  }
 return  res; 
